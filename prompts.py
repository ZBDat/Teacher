...
def get_scenario_prompt():
    ...
    return textwrap.dedent("""
        ...
        - 基于上述思路构思一个虚构国家/经济体，赋予它一个虚构国名与若干
          具体的经济问题（如通胀率、失业率、财政赤字、外债、汇率贬值压力、
          增长停滞等，可以用大致的数值/方向来描述）。
        - 重要：绝不能点名或暗示这取材于哪个真实国家/经济体，避免出现
          真实国名或明显指向真实事件的措辞，所有内容保持"虚构设定"口吻。
        - 输出尽量精简：用几句 Markdown 把该经济体的关键问题说清即可
          （通胀、增长、失业、财政、汇率等用简短的数值/方向点出），
          控制在 3-5 行，不要长篇大论，不要一次性给出解决方案。
    """)
...
def get_system_prompt(scenario=""):
    ...
    return textwrap.dedent(f"""
        ...
        - 每轮回答结构固定如下（务必简洁，避免长篇原因论述）：
          1. 用一句话简要评估该对策是否合理、立不立得住；
          2. 给出推演结论：只点出"政策——传导路径——结果"这条基本逻辑线，
             直接告诉用户该政策让哪个指标上升/下降、经济体现在怎么样了，
             不需要展开细枝末节的原因；
          3. 结合演化后的经济状态，向用户提出 1 个追问，引导其给出
             下一轮的对策。
